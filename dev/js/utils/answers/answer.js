...
class Answer {
  ...
  _generateCode() {
    var codeText = [];
    for(var i = 0; i < this.correctValues.length; i++) {
      var assertCode = this.names.map((name) => Variable.replaceVariables(this.correctValues[i][name]) != "" ? `Number(math.eval("${Variable.replaceVariables(this.correctValues[i][name])}").toFixed(${this.precision})) == inputValue['${name}']` : `(inputValue['${name}'] == "" || inputValue['${name}'] == null)` );
      if(i == 0) {
    ...
    for(var i = 0; i < this.commonErrors.length; i++) {
      var commonError = this.commonErrors[i];
      var failCode = this.names.map((name) => Variable.replaceVariables(commonError.values[name]) != "" ? `Number(math.eval("${Variable.replaceVariables(commonError.values[name])}").toFixed(${this.precision})) == inputValue['${name}']` : `(inputValue['${name}'] == "" || inputValue['${name}'] == null)`);
